...
HOW_TO_USE = [
    ...
    "Main's copy is published by the daily refresh only (issue #235); a pull request never commits it. "
    "A merged edit to data/schools.json shows here at the next refresh that publishes, or sooner via a "
    "build_only=true dispatch. `python tools/schools_review.py --dry-run --top 20` shows an edit's effect locally.",
...
class Recorder:
    ...
    def write(self, path: str | None = None) -> dict:
        """Write the report, reading the copy already at `path` as the previous one (`firstSeen`, `new`).

        In the refresh that copy is main's published report: the commit step restores it before its
        rebuild (issue #235). A name first seen on a day whose refresh is stopped by the build/validate
        gate publishes nothing that day, so it is dated to the next refresh that publishes."""
