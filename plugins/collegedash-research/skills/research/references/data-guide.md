...
- **Adding them up:** filter records, then count by status. Several clubs (or schools, or programs) are combined with OR within their list and AND across the lists: "players from club A or B who went to high school X, at program P" counts the records with `c` in {A, B}, `h` = X and `p` = P. The site's Pipelines page takes up to 3 values in each of its club, high-school and program boxes (a link with more keeps the first 3); the file itself answers any number. A former player's record carries both her club and her high school; report what the question needs, not the linked pair for a named person.
